...
public class FragmentLibrary {
        ...
        /**
         * Generate Regional Insight (Phase 2: Combinatorial Logic)
         * Combines climate + era + labor to explain "Why this region is different"
         * 
         * @param climateZone Climate zone code (e.g. "2A", "5A")
         * @param era         Era code (e.g. "PRE_1950")
         * @param laborMult   Labor cost multiplier (e.g. 1.45 = 145% of national avg)
         * @param metroName   Display name of metro (e.g. "Austin")
         * @param seed        Deterministic seed for variation selection
         * @return Contextual insight explaining regional uniqueness
         */
        public static String generateRegionalInsight(String climateZone, String era, double laborMult, String metroName,
                        long seed) {
                if (climateZone == null || era == null) {
                        return ""; // No insight if missing data
                }

                Random random = new Random(seed);
                StringBuilder insight = new StringBuilder();

                // Combinatorial Logic: Climate + Era
                String climateKey = getClimateKey(climateZone);

                StringBuilder climatePart = new StringBuilder();
                boolean hasClimateInsight = false;

                // Pattern 1: HIGH-IMPACT Climate + Era Combinations
                if (climateKey.equals("2A") && era.equals("PRE_1950")) {
                        climatePart.append(String.format(
                                        "%s's hot-humid climate (Zone 2A) accelerates deterioration of 1940s-era roofing materials by 15-20%% compared to dry climates, "
                                                        +
                                                        "while moisture infiltration in original wood-framed walls creates mold risks 3x higher than national averages. ",
                                        metroName));
                        hasClimateInsight = true;
                } else if (climateKey.equals("5A") && (era.equals("PRE_1950") || era.equals("1950_1970"))) {
                        climatePart.append(String.format(
                                        "Severe freeze-thaw cycles in %s (Zone 5A) cause concrete spalling and brick mortar degradation at rates 40%% higher than temperate zones, "
                                                        +
                                                        "especially critical for pre-1970s homes built before modern winterization standards. ",
                                        metroName));
                        hasClimateInsight = true;
                } else if (climateKey.equals("B") && era.contains("1980")) {
                        climatePart.append(String.format(
                                        "%s's intense UV exposure (Dry Zone) degrades 1980s-era composite siding and roof shingles 25%% faster than humid climates, "
                                                        +
                                                        "with thermal expansion stress causing premature fastener failure. ",
                                        metroName));
                        hasClimateInsight = true;
                } else if ((climateKey.equals("1") || climateKey.equals("2A"))
                                && (era.equals("PRE_1950") || era.equals("1950_1970"))) {
                        climatePart.append(String.format(
                                        "High humidity in %s compromises mid-century fiberglass insulation effectiveness by 30-40%%, "
                                                        +
                                                        "while outdated vapor barriers in pre-1970s construction invite chronic moisture intrusion. ",
                                        metroName));
                        hasClimateInsight = true;
                } else if (climateKey.equals("6") && !era.equals("2010_PRESENT")) {
                        String eraName = formatEraForNarrative(era);
                        climatePart.append(String.format(
                                        "%s's extreme cold (Zone 6) subjects older homes to thermal cycling stress that can crack foundation slabs and cause ice dam formation, "
                                                        +
                                                        "risks that were poorly understood during %s-era construction. ",
                                        metroName, eraName));
                        hasClimateInsight = true;
                }
                ...
                // Pattern 2: Labor Cost Context (Specific Percentages)
                int laborPercent = (int) Math.round(laborMult * 100);
                if (laborMult > 1.3) {
                        insight.append(String.format(
                                        "With labor costs at %d%% of national average, comprehensive restoration projects in %s require careful contractor vetting and strategic timing to manage budgets effectively.",
                                        laborPercent, metroName));
                } else if (laborMult < 0.9) {
                        insight.append(String.format(
                                        "Labor costs at just %d%% of national average make %s an exceptionally favorable market for extensive renovation work on historic properties.",
                                        laborPercent, metroName));
                } else {
                        insight.append(String.format(
                                        "Regional labor costs at %d%% of national average enable balanced project scoping without budget overruns.",
                                        laborPercent));
                }
        ...
        /**
         * Helper: Format era code for narrative use
         */
        private static String formatEraForNarrative(String era) {
                switch (era) {
                        case "PRE_1950":
                                return "pre-1950";
                        case "1950_1970":
                        ...
                        case "2010_PRESENT":
                                return "modern";
                        default:
                                return era.toLowerCase().replace("_", "-");
                }
        }
